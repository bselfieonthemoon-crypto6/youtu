...
## 删除确认:真实浏览器探测结论(2026-09-20)

探测脚本 `apps/web/scripts/check-delete-confirmation.mjs`,证据
`artifacts/delete-confirmation-browser/probe-2026-09-20T08-59-14-562Z.json`。
**总计 10 pass / 5 fail / 3 info**;阶段④(刷新)未到达——脚本在阶段②点击超时处崩溃。

| 阶段 | 结果 | 关键观测 |
| --- | --- | --- |
| ① 创建 | **4/4 PASS** | agent 提出危险操作确认;确认指向被播种的图(`sim-image-b8ab294c…`);卡片在浏览器渲染("需要确认危险操作"+"确认删除");**确认前图片仍在画布**(1 个活跃元素,revision 11) |
| ② 执行 | **0/4 PASS** | `确认删除` 按钮定位到 **0 个**;点击 20s 超时;图片未从画布删除;卡片未结算;确认块仍为 `confirmation_required` |
| ③ 过期 | **6 PASS / 1 FAIL** | 卡片带 `expiresAt`;**无后台定时器擅自删除**;TTL(10 分钟)走完 + 15s 图片仍未动;过期后确认按钮不再渲染;被拒点击与 60s 后均**未重放删除**;FAIL 的一条是"过期卡片无法再确认"——探针因按钮不再渲染而判定失败 |
| ④ 刷新 | 未执行 | 被阶段②的异常中断 |

**诚实结论**:

1. **安全的一半已被真浏览器验证**:创建必须先确认、确认前不删、过期不会擅自应用、也没有重放删除。
...
3. **下一步(未做)**:查看确认卡片组件的 DOM/可访问名,判定是"控件不是 button(可访问性与可测性问题)"
   还是"探针选择器过严";修好后重跑本探针拿到阶段②④的通过结论。**在拿到之前,不得把删除确认标为通过。**

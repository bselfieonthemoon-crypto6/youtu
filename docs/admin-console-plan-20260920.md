...
> **下一批 = B7（运行健康与平台设置）**。**死信重放/后台代用户重跑已决定不做**（2026-09-20 拍板，理由见决策点 3）。
...
| **B5 任务与渠道** | ✅ 已完成（B5a + B5b） | **B5a**：任务按状态/类型/工作区/错误码/时间筛选 + 详情（尝试次数、上游错误、关联会话画布、额度流水、管理记录、payload 预览）+ 取消排队中任务 + 死信标记已处置 + 卡住识别。**B5b**：跨工作区渠道目录 + 渠道自检/配置记录 + 按 `error_code` 的失败率（归属走 `provider_execution_snapshots`，无渠道记录的失败显式计数）。**重放已决定不做**（决策点 3） | 运维最常用；先做只读+取消/标记；"要不要花钱重放"最终结论是不做 | 中（取消影响在途任务） |
...
3. ~~**死信重放**~~ **已决定：不做（2026-09-20）**。后台不提供任何形式的"代用户重跑"。
   分工是：**个别失败由用户在前台自己重试**（失败占位本来就保留为重试入口）；如果是**我方问题**，
   走既有的"调整额度 + 原因 + 审计"补偿，由用户自己决定要不要重跑；后台只负责
   **定位**（失败率、错误码、卡住任务、渠道健康）与**止损**（取消、标记已处置）。
   理由：逐条重放会退化成"人肉重试按钮"，用户一多就不可维护，还会引入"替用户花钱"的特例；
   按原因批量重排虽然可扩展，但同样要定义"谁付费"，收益不足以撑起这套口径。
...
- **死信重放 / 后台代用户重跑**（含"按 error_code 批量重排"）：2026-09-20 决定不做。
  个别失败走前台自助重试，我方问题走改额度补偿；后台不做"替用户花钱"的动作。
  详见决策点 3。
...
- **技能图片上传偶发 400（B4a 路径）**：实现期间出现过**一次**上传被路由层参数校验拒绝（400），
  之后无法复现——字段顺序专项探测 12/12 + 12/12 通过，B4a/B6a 的完整端到端自检也反复全绿。
  代码路径本身有单测覆盖（含真实 multipart 上传与 8 种非法请求）。2026-09-20 决定**先忽略，日后再碰到再说**：
  重新排查的成本高于目前掌握的信息量，若再次出现，优先抓当时的请求原文与服务端日志。

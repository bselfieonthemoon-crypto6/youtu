...
> 见 `docs/admin-console-ops-bootstrap-20260920.md`。**下一批 = B5**。
...
| **B5 任务与渠道** | ⏭ 下一批 | 任务筛选/详情/取消/死信标记与处置 + 渠道自检记录与失败率（**重放不在本批**，涉及真实费用单独一批） | 运维最常用；先做只读+取消/标记，把"要不要花钱重放"留给你决定 | 中（取消影响在途任务；重放另议） |

...
> **下一步 = B6b（设计目录后台补全）**，其后 **B6c（资产与存储）**。死信重放仍未做，等你拍板（决策点 3）。
...
| **B6 内容与存储** | 🔶 拆三批：B6a ✅ / B6b ⏭ / B6c 待做 | **B6a（已完成）**：首页两个内容库（发现案例 + 示例）的分类、条目 CRUD、配图（URL）、排序与上下架；分类不提供删除（外键级联），排序走独立 reorder（`sort_order` 带唯一索引）。**B6b（下一步）**：设计目录后台补预览图/标签/上架状态/批量。**B6c**：资产与存储占用统计、孤儿盘点、待删队列、清理 | 内容维护可人工顶一阵；存储清理涉及删除，谨慎 | 中（删除类操作） |

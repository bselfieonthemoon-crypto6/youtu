...
> **B5 已拆成两批**：**B5a（任务目录/详情/取消/死信处置）已完成**，见 `docs/admin-console-b5a-20260920.md`；
> **下一批 = B5b（渠道自检记录与失败率）**。
...
| **B5 任务与渠道** | 🔶 B5a ✅ / B5b ⏭ | **B5a（已完成）**：任务按状态/类型/工作区/错误码/时间筛选 + 详情（含尝试次数、上游错误、关联会话画布、额度流水、管理记录、payload 预览）+ 取消排队中任务 + 死信标记已处置 + 卡住识别。**B5b（下一批）**：渠道自检记录与按 `error_code` 的失败率。**重放不在 B5**，涉及真实费用单独一批 | 运维最常用；先做只读+取消/标记，把"要不要花钱重放"留给你决定 | 中（取消影响在途任务；重放另议） |
| **B6 内容与存储** | 待做 | 首页案例/发现内容 CRUD + 设计目录补全 + 存储占用与孤儿清理 | 内容维护可人工顶一阵；存储清理涉及删除，谨慎 | 中（删除类操作） |

...
- 聊天侧原生画板创建/导出（I 的大半）：**用户已明确"先不需要做"（2026-09-20）**，
  因此不是待办而是确定的范围外项。本次只保留提示词与边界说明里的发现性（告诉用户"可编辑的原生画板要由你在画板上手动添加与编辑"），
  且不得把它倒推成"聊天里能直接改画板"的承诺。若将来重启此需求，需要先做产品决策（创建入口、导出物、权限与计费口径）。

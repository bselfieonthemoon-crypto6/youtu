-- A direct Generate click has its own durable identity. It is not a chat approval.
CREATE TABLE public.node_image_submissions (
  created_by uuid NOT NULL REFERENCES auth.users(id),
  request_id uuid NOT NULL,
  canvas_id uuid NOT NULL REFERENCES public.canvases(id) ON DELETE CASCADE,
  element_id text NOT NULL CHECK (length(element_id) BETWEEN 1 AND 200),
  input jsonb NOT NULL,
  job_id uuid UNIQUE REFERENCES public.background_jobs(id) ON DELETE SET NULL,
  created_at timestamptz NOT NULL DEFAULT now(),
  PRIMARY KEY (created_by, request_id)
);
ALTER TABLE public.node_image_submissions ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON public.node_image_submissions FROM PUBLIC, anon, authenticated;
GRANT SELECT ON public.node_image_submissions TO authenticated;
GRANT ALL ON public.node_image_submissions TO service_role;
CREATE POLICY node_image_submission_read ON public.node_image_submissions FOR SELECT TO authenticated
USING (created_by = auth.uid() AND EXISTS (SELECT 1 FROM public.canvases c WHERE c.id = canvas_id));

-- Server-only: pricing/model checks happen at the HTTP boundary. Recheck ownership,
-- frozen model revision, node identity and the ledger atomically with publication.
CREATE FUNCTION public.loomic_submit_node_image(
...
BEGIN
  IF p_user IS NULL OR p_request IS NULL OR p_canvas IS NULL OR p_element IS NULL
    ...
    OR (SELECT count(*) FROM jsonb_object_keys(p_input)) <> 4
    ...
    OR p_input->>'quality' NOT IN ('standard','hd','ultra') THEN
  ...
  IF element->>'isDeleted' = 'true' OR element#>>'{customData,type}' IS DISTINCT FROM 'image-generator'
    ...
    OR frozen->>'quality' IS DISTINCT FROM p_input->>'quality' THEN
    RAISE EXCEPTION 'node_submission_conflict';
  END IF;

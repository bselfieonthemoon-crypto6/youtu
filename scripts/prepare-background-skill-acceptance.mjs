...
const dir='artifacts/background-skill-20260915';await mkdir(dir,{recursive:true});
...
const skills=await call('/api/workspaces/skills');const skill=skills.skills.find(s=>s.slug==='background-removal');assert(skill);console.log(JSON.stringify({slug:skill.slug,enabled:skill.enabled,version:skill.version,readiness:skill.readiness}));
...
if(!fixture){const project=(await call('/api/projects','POST',{name:'QA 去背景技能自动调用',description:'验证当前模型、技能自动读取、原图透明编辑与结果入画布'})).project;
 ...
 if(!sessionId){const created=await db.from('chat_sessions').insert({canvas_id:project.primaryCanvas.id,created_by:ownerId,title:'去背景技能验证',thread_id:'thread_'+crypto.randomUUID()}).select('id').single();assert.ifError(created.error);sessionId=created.data.id;}

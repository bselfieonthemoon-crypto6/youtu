...
const dir='artifacts/historical-reference-20260915';await mkdir(dir,{recursive:true});
...
if(!fixture){const project=(await call('/api/projects','POST',{name:'QA 跨轮参考图',description:'验证上传参考图在下一轮无需重传'})).project;
 ...
 if(!sessionId){const created=await db.from('chat_sessions').insert({canvas_id:project.primaryCanvas.id,created_by:ownerId,title:'跨轮参考图验证',thread_id:'thread_'+crypto.randomUUID()}).select('id').single();assert.ifError(created.error);sessionId=created.data.id;}

  ...
  return tool(
    async (input: ImageGenerateInput, config) => {
      ...
      if (deps.proposalStore) {
        ...
        const latest = await deps.proposalStore.latest(configurable);
        if (
          !frozenInput.target &&
          latest?.input.target &&
          !/无限画布|主画布|画板外/.test(String(configurable.user_prompt ?? ""))
        ) {
          return {
            error: "design_target_required",
            summary:
              "本对话上次生成目标是设计画板。请 inspect_design 后明确 target；若要切换到无限画布，请先询问用户。未开始生成。",
          };
        }
        ...
        return {
          ...
          confirmation,
          summary:
            "方案已保存，尚未生成。请说明设计方案并询问用户是否确认；修改需求必须重新创建方案。",
        };
      }

...
describe("read_conversation_evidence", () => {
  it("binds authenticated identity outside model arguments and accepts only bounded evidence selectors", async () => {
    ...
    const tool = createConversationEvidenceTool({ service: { readEvidence }, scope: original });
    ...
    await expect(tool.execute({ workspaceId: id(9), messageIds: [id(6)] }, toolExecutionContext({}))).rejects.toThrow();
  ...
  it("accepts controlled cursors and rejects mixed cursor/id selectors", async () => {
    ...
    const tool = createConversationEvidenceTool({ service: { readEvidence }, scope });
  ...
  it("reports revoked access and does not restore approval claims from a summary", async () => {
    ...
    const output = await createConversationEvidenceTool({ service: { readEvidence }, scope }).execute({}, toolExecutionContext({}));

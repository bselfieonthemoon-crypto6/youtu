...
describe("every loaded guide comes back in full", () => {
  ...
  it("has no 'already in the instructions' case left to elide", async () => {
    // The runtime injects no guide text, so a tool result is the ONLY way a body
    // reaches the model. There is therefore no marker, no elision and no
    // `alreadyPreloaded` field: any of those would mean the model was handed
    // identity instead of the method it asked for.
    ...
    const composed = await compose.execute({ deliverable: "Logo", stage: "design", primary: entry.name, helpers: [] },
      toolExecutionContext({}));
    expect(composed).toMatchObject({ status: "composed", primary: { instructions: entry.content } });
    expect(composed).not.toHaveProperty("alreadyPreloaded");
    // Role and responsibility are composition facts, not guide text: keep them.
    expect((composed as any).primary.role).toBe("domain");
  });
});
